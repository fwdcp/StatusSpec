...
FreezeInfo::FreezeInfo(std::string name) : Module(name) {
	...
	enabled = new ConVar("statusspec_antifreeze_enabled", "0", FCVAR_NONE, "enable freeze info panel", [](IConVar *var, const char *pOldValue, float flOldValue) { g_ModuleManager->GetModule<FreezeInfo>("Freeze Info")->ToggleEnabled(var, pOldValue, flOldValue); });
