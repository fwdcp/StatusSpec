...
CameraSmooths::CameraSmooths(std::string name) : Module(name) {
	...
	enabled = new ConVar("statusspec_camerasmooths_enabled", "0", FCVAR_NONE, "enable smooth glide to new view on camera switch", [](IConVar *var, const char *pOldValue, float flOldValue) { g_ModuleManager->GetModule<CameraSmooths>("Camera Tools")->ToggleSmoothCameraSwitchesEnabled(var, pOldValue, flOldValue); });
